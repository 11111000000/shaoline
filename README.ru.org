...
#+begin_quote
“Модлайн, который можно увидеть, не истинный модлайн.”
...
3. /Мигает echo-area!/
   В Emacs 30+ Shaoline ставит ~clear-message-function~: гард возвращает
   ~dont-clear-message~, если в echo уже лежит наша строка (свойство
   ~shaoline-origin~), и пакеты вроде ~agent-shell-active-message-hide~
   больше не могут стереть модель-лайн. На старых Emacs этого хука
   нет — там работает advice-обёртка ~shaoline--advice-preserve-empty-message~.
   Если мерцание всё же осталось, включите ~shaoline-debug~ и смотрите
   стек вызовов.
4. /Работает ли в TTY?/
