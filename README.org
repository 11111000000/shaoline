...
3. /Echo area flickers!/
   In Emacs 30+ Shaoline installs ~clear-message-function~ so
   packages that spam ~(message nil)~ (e.g. ~agent-shell-active-message-hide~)
   cannot erase the modeline: the guard returns ~dont-clear-message~
   whenever the echo content carries the ~shaoline-origin~ property.
   On older Emacps there is no such hook, so the advice-trampoline
   blocks the empty message instead. If you still see flicker, turn on
   ~shaoline-debug~ to capture the offending call.
4. /TTY support?/
...
• Token bucket shapes traffic; light-updates rate-limited further.
